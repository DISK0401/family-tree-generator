...
describe('FamilyTreeCanvas: 表示モードの3値化(8.1, 8.2)', () => {
  it('既定は折りたたみ表示で、3つのボタンがそれぞれ切り替えられる', () => {
    render(<FamilyTreeCanvas selectedPersonId={null} onSelectPerson={() => {}} />)

    const collapsedButton = screen.getByRole('button', { name: '折りたたみ表示' })
    const fullButton = screen.getByRole('button', { name: '全体表示(家系ごと)' })
    const connectedButton = screen.getByRole('button', { name: 'つながった全体表示' })
...
describe('FamilyTreeCanvas: connectedモードでの描画系の入れ替え(8.3)', () => {
  it('つながった全体表示ではfamily-chartのキャンバスが隠れ、PedigreeCanvasが描画される', () => {
    const { container } = render(<FamilyTreeCanvas selectedPersonId={null} onSelectPerson={() => {}} />)

    // 既定(折りたたみ表示)ではfamily-chartのコンテナが見えており、PedigreeCanvasは無い
    expect(container.querySelector('.f3.tree-canvas-root')).not.toHaveAttribute('hidden')
    expect(container.querySelector('.pedigree-canvas-root')).not.toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: 'つながった全体表示' }))

    // 切り替えるとfamily-chart側は隠れ(DOM自体は保持。FamilyTreeCanvas.tsxのコメント参照)、
    // PedigreeCanvasが現れる
    expect(container.querySelector('.f3.tree-canvas-root')).toHaveAttribute('hidden')
...
describe('FamilyTreeCanvas: connectedモードでのトレイ(8.4)', () => {
  it('折りたたみ表示では図に現れていない人物の一覧が出るが、connectedでは出ない', () => {
    ...
    expect(screen.queryByLabelText('図に現れていない人物')).not.toBeInTheDocument()
  })
})

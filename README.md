...
- **表示設定**: 設定メニューから、人物カードに表示する生年月日・没年月日の粒度(年のみ/年月まで/年月日まで)、表示形式(西暦/和暦)、表示する項目(姓・名・ふりがな・生年月日・没年月日・出生地・没地・年齢・性別アイコンを個別にオン/オフ。例: 姓を非表示にして名だけ表示する)を選べる。端末ローカルの表示設定であり、GEDCOM/JSONのエクスポート内容には影響しない。

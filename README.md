...
- **全体表示モード**: キャンバス左下のトグルで、折りたたみ(クリックのたびに視点が移動する既定の表示)を止め、記録されている家系全体を1枚の図に表示するモードに切り替えられる。実親・養親の両方が記録されている人物のように複数の親家族を持つ人物や、互いに血縁・婚姻関係のない複数の家族クラスタも含め、全員を漏れなく表示する(該当する人物は、それぞれの家系の下にカードが複製されて表示される)。折りたたみ表示では、選択中の視点から辿れない範囲にいる人物がいるカードに人数バッジが表示され、クリックするとそちら側へ視点を切り替えて確認できる。

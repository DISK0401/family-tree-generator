...
- **全体表示モード**: キャンバス左下のトグルで、折りたたみ(クリックのたびに視点が移動する既定の表示)を止め、選択人物の兄弟姉妹も含めて描画可能な最大範囲を常に表示するモードに切り替えられる。その先に非表示の人物がいるカードには人数バッジが表示され、クリックするとそちら側へ視点を切り替えて確認できる(養子縁組など複数の親を持つ人物は、家系図描画ライブラリの制約により実親・養親を同時に1枚の図へ収めることができないため、全体表示モードでもバッジ経由での確認が必要になる)。

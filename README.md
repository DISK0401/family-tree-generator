...
ランディングの「サンプルで完成イメージを見る」から、以下の5パターンをエディタで開ける(`src/samples/`)。

| サンプルID | 題材 | 見せるパターン |
|---|---|---|
| `tokugawa-ieyasu` | 徳川家康 | 複数の配偶者(正室・継室・側室) |
| `tokugawa-shoguns` | 徳川将軍15代 | 多世代(15代32名)・分家(紀州・一橋・水戸)からの継承・実父と養父の併存 |
| `natsume-soseki` | 夏目漱石 | 養子縁組(実父母と養父母) |
| `shibusawa-eiichi` | 渋沢栄一 | 死別後の再婚・旧字体表記「澁澤榮一」 |
| `modern-family` | 現代の家族(架空) | 三世代の記録・和暦⇄西暦 |

日付は和暦の原文を保持し、構造化日付はその原文を日付入力欄に入れたときにアプリが記録する値と同じにする。旧暦(明治6年の改暦以前)は本体と同じ名目方式で、和暦の年月日をそのままの数字で西暦フィールドへ写す(西暦表示は目安であり、年をまたぐ換算とは一致しない。例: 徳川家康の生年 天文11年12月26日 = グレゴリオ暦1543年1月31日を `1542-12-26` として記録する)。閏月は原文のみに保持し、構造化日付は同じ番号の月へ畳む。改暦後の日付は実際のグレゴリオ暦の年月日を記録する。
...
- **人物情報の編集**: サイドパネルで氏名(姓・名を別々に保持し正規化しない)・ふりがな・性別・出生順(家族内での出生順。任意)・生年月日/没年月日・メモを編集できる。出生順は「長男」「次男」のような性別付きラベルではなく性別非依存の順位として持ち、長男/次男/長女/次女…といった続柄ラベルは表示のたびに性別と組み合わせて導き出す(性別不明の兄弟はどちらの数にも数えない)。これにより、古い戸籍で長男の記載が欠け次男以降しか記録されていない場合でも、名前・生年月日が不明な人物として「出生順1」だけを記録しておけば、以降の兄弟が正しい続柄で表示・整列される。日付は和暦・西暦(年月日表記・ハイフン区切り・区切りなし8桁数字のいずれも)のどちらで入力しても、入力と同時にもう一方の表記が表示される(例: 「昭和39年10月10日」「19641010」→「1964年10月10日」)。「頃」「以前」「以後」などの不確実な日付表現にも対応。和暦の元号は明治以降に限らず、大化(645年)から令和までの日本の元号全体に対応する(南北朝期は北朝系列の単一系列。南朝の元号と、元弘と並立した正慶には非対応)。明治6年(1873年)のグレゴリオ暦採用より前の日付は、旧暦(太陰太陽暦)の年月日をそのままの数字で記録し、西暦への厳密な換算は行わない(日は1〜30日の範囲で受け付け、存在しない旧暦日の厳密な検証はしない目安の値として扱う)。フォームに未確定の変更がある状態で他の人物を選択しようとすると確認ダイアログを表示し、変更の破棄・保存のどちらかを選べる(確定操作はEnterキーでも実行できる)。
...
- **自動整列と縦書き表示**: 自動レイアウトで、人物追加のたびに図全体が視界に収まるよう再整列する(折りたたみ表示・全体表示(家系ごと)は [family-chart](https://github.com/donatso/family-chart)、つながった全体表示は自前のレイアウタによる)。人物カードの氏名は縦書き・明朝体で表示し、性別を色と形状(男=四角・女=丸・不明=破線ひし形)で、生年月日が判明していれば現年齢(故人は没年齢)も表示する。出生順が設定されている人物には、性別インジケーター・故人マーカー(†)に続けて「長男」「次男」等の続柄ラベルを表示する。兄弟の左右の並び順は、出生順が設定されていればそれを優先し(片方だけ設定されている場合もそちらを優先)、いずれにも設定がなければ従来どおり生年月日→氏名の順で決まる。
...
- **人物一覧の表(表形式ビュー)と Excel ライク編集モード**: 画面上部の「図 / 表」で切り替えると、登録されている全人物を1行1人の表で一覧できる。列は右側パネルで編集できる項目(姓・名・ふりがな・性別・出生順・生没年月日・生没地・メモ)+閲覧用の配偶者名。**列見出しからすべての列を並べ替えでき**(押すたびに昇順→降順→解除。未入力の値は常に末尾)、**列見出しの直下の欄で列ごとに絞り込める**(性別は選択式、それ以外は部分一致。複数列を指定すると全条件を満たす行だけが残る)。生年月日・没年月日の絞り込みは表示中の書式に縛られず、**和暦でも西暦でも(「平成6年10月01日」「1994-10-01」「19941001」いずれでも)同じ人物がヒットする**。「1994」「平成6年」のように年だけ、「1960〜1965」のように範囲でも絞れる。氏名・ふりがなを横断して探す検索欄も別にあり、絞り込みは「絞り込みを解除」で一括して戻せる。「編集」モードに切り替えると表計算ソフトのように使える。
...
- 氏名・続柄・関係種別・日付は `docs/gedcom-mapping.md` の対応表に従ってGEDCOMタグへマッピングされる(ふりがなは拡張タグ `_KANA_SURN`/`_KANA_GIVN`、事実婚等の関係種別は拡張タグ `_FAM_KIND`、出生順は拡張タグ `_BIRTH_ORDER`)。

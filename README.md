...
- **図に現れていない人物の一覧**: 関係を持たない人物や、本体の家系から切り離された人物は既定の折りたたみ表示では図に描けないため、キャンバス下部の一覧に並べる。クリックすると通常どおり編集パネルが開く。該当する人物がいないときは表示されない(全体表示モードでは全員が図に現れるため常に空になる)。
- **自動整列と縦書き表示**: [family-chart](https://github.com/donatso/family-chart) による自動レイアウトで、人物追加のたびに図全体が視界に収まるよう再整列する。人物カードの氏名は縦書き・明朝体で表示し、性別を色と形状(男=四角・女=丸・不明=破線ひし形)で、生年月日が判明していれば現年齢(故人は没年齢)も表示する。
- **全体表示モード**: キャンバス左下のトグルで、折りたたみ(クリックのたびに視点が移動する既定の表示)を止め、記録されている家系全体を1枚の図に表示するモードに切り替えられる。実親・養親の両方が記録されている人物のように複数の親家族を持つ人物や、互いに血縁・婚姻関係のない複数の家族クラスタも含め、全員を漏れなく表示する(該当する人物は、それぞれの家系の下にカードが複製されて表示される)。折りたたみ表示では、選択中の視点から辿れない範囲にいる人物がいるカードに人数バッジが表示され、クリックするとそちら側へ視点を切り替えて確認できる。どの人物からも辿れず視点の切り替えでは到達できない人物は、バッジではなく「図に現れていない人物」の一覧に並ぶ。

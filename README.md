...
- **人物一覧の表(表形式ビュー)と Excel ライク編集モード**: 画面上部の「図 / 表」で切り替えると、登録されている全人物を1行1人の表で一覧できる。列は右側パネルで編集できる項目(姓・名・ふりがな・性別・生没年月日・生没地・メモ)+閲覧用の配偶者名で、氏名・生年月日での並べ替えと氏名・ふりがなでの絞り込みができる(未入力の生年月日は常に末尾)。「編集」モードに切り替えると表計算ソフトのように使える。

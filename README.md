...
- **人物一覧の表(表形式ビュー)と Excel ライク編集モード**: 画面上部の「図 / 表」で切り替えると、登録されている全人物を1行1人の表で一覧できる。列は右側パネルで編集できる項目(姓・名・ふりがな・性別・生没年月日・生没地・メモ)+閲覧用の配偶者名。**列見出しからすべての列を並べ替えでき**(押すたびに昇順→降順→解除。未入力の値は常に末尾)、**列見出しの直下の欄で列ごとに絞り込める**(性別は選択式、それ以外は部分一致。複数列を指定すると全条件を満たす行だけが残る)。氏名・ふりがなを横断して探す検索欄も別にあり、絞り込みは「絞り込みを解除」で一括して戻せる。「編集」モードに切り替えると表計算ソフトのように使える。

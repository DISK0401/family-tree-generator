...
## 10. 表形式ビュー(person-table-editor、実装後に発覚した抜け)

- [x] 10.1 `src/organisms/person-table/columns.ts`の`PERSON_TABLE_COLUMNS`に「出生順」列を追加する(性別列の直後、`PersonEditForm`と同じ並び)。1以上の整数以外はエラー、空文字で未設定に戻す
- [x] 10.2 `columns.test.ts`に出生順列の表示・編集・不正値のテストを追加する
- [x] 10.3 `PersonTableView.test.tsx`の列数ハードコード(`toHaveLength(11)`)を`12`へ更新する
- [x] 10.4 `README.md`の表形式ビューの列一覧に出生順を追記する
- [x] 10.5 実画面(`npm run dev`)で表モードに出生順列が表示され、編集できることを確認する

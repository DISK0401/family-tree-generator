...
- [x] 6.1 README.mdに現状ズーム/パンの操作方法の説明が無いことを確認し、追記が必要かどうかを判断する(完了条件: 追記する場合は該当箇所を更新、不要と判断した場合はその旨をPR説明に記す) — README.mdは機能一覧(データモデル・操作の到達可能性)を説明する構成で、マウス/トラックパッド等の入力デバイス単位のジェスチャ説明は元々どの項目にも無い。本changeは内部の入力イベント処理の修正であり新しい到達可能な機能を追加するものではないため、追記不要と判断した

## 7. 実機確認とarchive

- [x] 7.1 ユーザーが手元のMac Chromeで、3表示モード(折りたたみ表示・全体表示(家系ごと)・つながった全体表示)それぞれについて、2本指パン・Ctrl(⌘)+ホイール/ピンチによるズーム・ブラウザの「戻る」誤爆の有無を確認する(design.md「archiveのタイミング」参照)
- [x] 7.2 7.1の結果を踏まえたユーザーのarchive指示を受けてから、このchangeをarchiveする

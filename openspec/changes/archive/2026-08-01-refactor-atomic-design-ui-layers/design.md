...
全 4 段の完了時点で **計算後スタイルの差分 0 件**(16 画面 / 176 要素 / 40 プロパティ)。各段の完了時にも同じ比較を行い、いずれも差分 0 件で通している。

スクリーンショットは 16 枚中 15 枚がバイト単位で一致し、`light-panel-relation` の 1 枚だけが相違した。これは**撮影のノイズであり回帰ではない**ことを確認済み: 同一コードで 2 回撮った画像同士を比べても `light-panel-relation`(氏名欄が autoFocus でキャレットが点滅する)と `dark-dialog`(フォーカス表示)は一致しない。テキストキャレットの点滅位相が撮影時刻に依存するため。

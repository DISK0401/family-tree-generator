...
- **実機のMac Chromeトラックパッドでの確認要否**: 「戻る」誤爆の防止とピンチの体感は、Playwrightのsynthetic wheelイベントでは`ctrlKey`分岐・`scaleBy`/`translateBy`呼び出しのロジックは検証できるが、実際のOSジェスチャ認識・慣性スクロールのフェーズ差までは再現できない。CLAUDE.mdの既定(archiveのゲートはローカル検証で足り、デプロイ環境での確認はユーザーがそのつど指示する)に従うと、本changeはローカルのunit/e2eテストのみでarchiveしてよいことになるが、この不具合の性質上、実機確認をしないと「本当に直ったか」を確信できない。実機確認(dev環境デプロイ後、ユーザー自身のMacで2本指パン・ピンチ・戻る誤爆の有無を確認)を必須にするか、それともロジックのテストが通ればarchiveしてよいか、方針を確認したい

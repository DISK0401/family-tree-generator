...
- [x] 1.1 `src/layout/coordinates.ts` の `CARD_SIZE` を新しい高さ・幅に変更する(D3で決めた初期値。氏名3文字が全項目表示時でも等倍で収まることを1.2の目視確認で調整)。完了条件: `coordinates.test.ts` が新寸法を前提に更新され通過する — 初期値128×148で着手し、4群の実測で不足が判明したため最終的に136×180へ改めた(詳細は4.1参照)。テストはCARD_SIZEを定数参照しているため修正不要、そのまま通過を確認
...
- [x] 4.1 `visibleCardFields` を全てオンにした状態のカードHTMLをテストで生成し、`person-card.css` のパディング・行間トークンを目視確認しながら微調整する(必要なら `.tree-card` の `gap` や `padding-bottom` を調整) — **実装中に重大な発見**: 姓名3文字(無縮小上限)+全項目オン+和暦+出生地/没地両方、という最も厳しい組み合わせを実際にブラウザ(Playwright)でレンダリングしたところ、`.tree-card-name-row`(`flex:1; min-height:0`)が他の行に押されて必要な高さ(3文字で約55px)を確保できず、氏名の縦書きテキストが実際に生没年月日の行へはみ出して重なることを確認した(128×148では約25px不足)。`CARD_SIZE`を136×180へ拡大し、`gap`を`--space-1`→`--space-2`、下端パディングも`--space-2`に広げることで解消。この組み合わせで実測した結果、はみ出しが消え、名前列と生没年月日の間に8pxの余白ができることを確認した
- [x] 4.2 開発サーバで、ふりがな・生年月日・没年月日・年齢・生没地をすべてオンにした人物カード(画像2相当の再現データ)をスクリーンショットで確認し、要素間の余白が視認できることを確認する — 4.1の実測・スクリーンショットで確認済み。あわせて既定の表示項目数(画像2より少ない場合)でも間延びして見えないことをサンプルデータで確認した

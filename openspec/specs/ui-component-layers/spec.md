# ui-component-layers Specification

## Purpose
TBD - created by archiving change refactor-atomic-design-ui-layers. Update Purpose after archive.
## Requirements
...
構造の再編そのものは UI の振る舞い・視覚表現・アクセシビリティ名を変更してはならない(MUST NOT)。例外は「既存の不統一を基本形へ揃える」ことを明示的に決めた箇所に限り、その一覧と観測された差分は記録されなければならない(SHALL。下記「既存の不統一の是正」を参照)。

移動と層の再配置に伴うテストの変更は、import パス・モジュールのモック指定・自身の位置を指すパスに限られなければならない(SHALL)。振る舞いのアサーションを緩めてはならない(MUST NOT)。
...
#### Scenario: 是正箇所以外に視覚回帰がないことを実測で確認できる

- **GIVEN** 主要画面(空状態・編集パネル・関係追加フォーム・表形式ビュー・3 表示モード・設定メニュー・確認ダイアログ・インポート/エクスポート・ランディング)
- **WHEN** ライト/ダーク両テーマで各要素の計算後スタイル(`getComputedStyle` と寸法)を採取し、変更前(`develop`)の同一画面と機械比較する
- **THEN** 差分は「既存の不統一の是正」で列挙した箇所に限られ、それ以外の要素は 1 プロパティも変わらない

### Requirement: 既存の不統一の是正

基本形へ揃えることを明示的に決めた箇所は、意図した視覚変更として実測値とともに記録されなければならない(SHALL)。是正の対象は以下に限る。

| 箇所 | 是正前 | 是正後 |
| --- | --- | --- |
| 設定メニューの `<select>`(表示形式・生年月日・没年月日) | 書体指定がなく UA 既定(Arial 等のシステム書体) | 基本形 `.field .field--sm` を当ててゴシック体へ統一(高さ 28px → 32px) |
| インポート/エクスポートの「エクスポート」ボタン | クラスを持たず UA 既定のボタン(灰色・`outset` 枠・全幅) | 基本形 `.btn--outline` を当て、同ダイアログの「閉じる」と同じ語彙・余白へ統一(全幅 → 内容幅) |
| 人物カード `.tree-card` | 面(地・枠・角丸・影)を自前で宣言 | `.surface--raised` の上に載せる(**値が同一のため視覚差分なし**) |

#### Scenario: 設定メニューの選択欄が周囲の文字と同じ書体で描かれる

- **GIVEN** 設定メニューを開いている
- **WHEN** 表示形式・生年月日・没年月日の `<select>` の計算後の `font-family` を確認する
- **THEN** アプリのゴシック体(`--font-gothic`)であり、UA 既定のシステム書体ではない

#### Scenario: エクスポートボタンが同じダイアログの他のボタンと同じ語彙で描かれる

- **GIVEN** インポート/エクスポートのダイアログを開いている
- **WHEN** 「エクスポート」と「閉じる」の枠・角丸・文字色を比較する
- **THEN** どちらも基本形 `.btn--outline` の枠(1px・`--line-strong`)と角丸(`--radius`)を持ち、UA 既定の `outset` 枠が残っていない

#### Scenario: 人物カードの面が基本形と同一の値であることを保証できる

- **GIVEN** `.tree-card` が `.surface--raised` の上に載っている
- **WHEN** ライト/ダーク両テーマでカードの計算後スタイルを是正前と比較する
- **THEN** 地・枠・角丸・影のいずれも差分がない(基本形へ寄せても見た目が変わらないことが、値がトークンで揃っていた証拠になる)
